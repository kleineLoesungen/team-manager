...
function maybe_migrate_db(PDO $pdo): void {
    ...
    // Migration 028: is_system flag + nullable team_id on columns + RLS update.
    // Introduces admin-managed system columns (cross-team, immutable by coordinators).
    // System columns: team_id = NULL, list_id = NULL, is_system = TRUE.
    // Coordinator global columns: team_id = X, list_id = NULL, is_system = FALSE (unchanged).

    // Step 1: make team_id nullable (detect current constraint state first)
    try {
        $team_id_nullable = $pdo->query(
            "SELECT is_nullable FROM information_schema.columns
             WHERE table_schema = '{$schema}' AND table_name = 'columns' AND column_name = 'team_id'"
        )->fetchColumn();
        if ($team_id_nullable === 'NO') {
            $pdo->exec("ALTER TABLE {$schema}.columns ALTER COLUMN team_id DROP NOT NULL");
        }
    } catch (PDOException $e) {
        error_log('team-manager: migration 028 step 1 skipped — ' . $e->getMessage());
    }

    // Step 2: add is_system column (IF NOT EXISTS — idempotent)
    try {
        $pdo->exec(
            "ALTER TABLE {$schema}.columns ADD COLUMN IF NOT EXISTS is_system BOOLEAN NOT NULL DEFAULT FALSE"
        );
    } catch (PDOException $e) {
    ...
    try {
        ...
        error_log('team-manager: migration 028 step 5a skipped — ' . $e->getMessage());
    }

    // Step 5b: update columns_delete — prevent coordinators from deleting system columns
    try {
        $pdo->exec("DROP POLICY IF EXISTS columns_delete ON {$schema}.columns");
        $pdo->exec("CREATE POLICY columns_delete ON {$schema}.columns FOR DELETE USING (
            current_setting('app.is_admin', true) = 'true'
        ...
        error_log('team-manager: migration 028 step 5b skipped — ' . $e->getMessage());
    }

    // Step 5c: update columns_insert — ensure coordinator inserts do not create system columns
    try {
        $pdo->exec("DROP POLICY IF EXISTS columns_insert ON {$schema}.columns");
        $pdo->exec("CREATE POLICY columns_insert ON {$schema}.columns FOR INSERT WITH CHECK (
            current_setting('app.is_admin', true) = 'true'
        ...
        error_log('team-manager: migration 028 step 5c skipped — ' . $e->getMessage());
    }
}

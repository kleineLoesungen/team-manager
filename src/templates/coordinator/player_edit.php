<?php
// src/templates/coordinator/player_edit.php — Edit player form (coordinator)
// Variables: $player (array), $clubs (array), $player_id (int), $error (string)
?>
<?php if ($error): ?>
...
<div class="mb-3">
    <a href="/coordinator/players/<?= (int)$player_id ?>" class="btn btn-sm btn-outline-secondary">
        <i class="bi bi-arrow-left me-1"></i>Zurück
    </a>
</div>

<form method="POST" action="/coordinator/players/<?= (int)$player_id ?>/edit" novalidate>
    <?= csrf_field() ?>
    <div class="row g-3">
        <div class="col-6">
            ...
            <input type="text" id="first_name" name="first_name" class="form-control min-touch"
                   value="<?= e($player['first_name']) ?>" required autofocus>
        </div>
        <div class="col-6">
        ...
        <div class="col-12">
            ...
            <select id="club_id" name="club_id" class="form-select min-touch">
                ...
                <option value="<?= (int)$c['id'] ?>"
                    <?= ((int)($player['club_id'] ?? 0) === (int)$c['id']) ? 'selected' : '' ?>>
                    <?= e($c['name']) ?>
                </option>
                <?php endforeach; ?>
            </select>

<?php
// src/moderator/columns_create_handler.php — POST /moderator/columns/create (LIST-02)
...
if (empty($name)) {
    redirect('/moderator/columns?error=' . urlencode('Name ist erforderlich.'));
}
if (!in_array($data_type, ['boolean', 'number'])) {
    redirect('/moderator/columns?error=' . urlencode('Ungültiger Typ. Globale Spalten erlauben nur Ja/Nein oder Zahl.'));
}
...
try {
    ...
    redirect('/moderator/columns?success=1');
} catch (PDOException $e) {
    error_log('Global column create error: ' . $e->getMessage());
    redirect('/moderator/columns?error=' . urlencode('Ein Fehler ist aufgetreten.'));
}

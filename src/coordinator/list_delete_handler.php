<?php
// src/coach/list_delete_handler.php — POST /coordinator/lists/{id}/delete (LIST-DELETE)
...
if ($confirm !== 1) {
    ...
    render_coach_page('Liste löschen', 'lists', function() use ($list) {
        ...
        <form method="POST" action="/coordinator/lists/<?= (int)$list['id'] ?>/delete">
            ...
            <a href="/coordinator/lists/<?= (int)$list['id'] ?>/settings" class="btn btn-outline-secondary ms-2 min-touch">Abbrechen</a>
        </form>
        <?php
    });
    return;

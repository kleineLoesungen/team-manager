<?php
// src/coach/list_column_create_handler.php — POST /coordinator/lists/{id}/columns/create (LIST-03)
...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    redirect('/coordinator/lists/' . $list_id);
}
...
if (empty($name)) {
    redirect('/coordinator/lists/' . $list_id . '?error=' . urlencode('Spaltenname ist erforderlich.'));
}
// Local columns allow boolean, number, text (unlike global columns)
if (!in_array($data_type, ['boolean', 'number', 'text'])) {
    redirect('/coordinator/lists/' . $list_id . '?error=' . urlencode('Ungültiger Spaltentyp.'));
}
...
try {
    ...
    redirect('/coordinator/lists/' . $list_id . '?error=' . urlencode('Ein Fehler ist aufgetreten.'));
}
